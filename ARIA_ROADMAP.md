...
| **Security headers / CSP** | ✅ Done (Report-Only), July 1, 2026 | `next.config.mjs` now ships a real CSP built from an audit of every external domain the frontend calls (Google OAuth redirect, Sui fullnode, zkLogin prover, Walrus publisher/aggregator, the Railway API via `NEXT_PUBLIC_API_URL`, `images.unsplash.com` for demo photos, `img-src https:` kept open since host-imported listings can carry photos from arbitrary CDNs). Seal's key-server URLs are resolved dynamically on-chain, not hardcoded — covered by an `*.mystenlabs.com` wildcard. Shipped as `Content-Security-Policy-Report-Only` first, matching the original plan: watch for violations against real traffic for a few days, then flip to enforcing. Stripe.js isn't allowlisted yet since the frontend doesn't load it (Stripe integration is still a stub — see the Stripe webhook backlog item); add `js.stripe.com`/`api.stripe.com` when that ships. |
