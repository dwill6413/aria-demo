...
| No automated tests | Medium | Backend unit tests in `escrow.test.mjs` — **79 passing** (incl. resale verifier + `isObjectMutated` gRPC-shape tests). Move suite **52 tests** (incl. 8 resale cases). **No frontend tests** — and the resale UI (list/cancel/buy/market modal in `bookings.jsx`, host toggle in `host.jsx`, ID `idState` field in `profile.jsx`) is entirely unverified by automated tests. **Route-level tests are now UNBLOCKED** — the `server.mjs` split (R1) shipped July 1, 2026; next step is scoping them, highest-risk routes first (escrow confirm/rebuild, payments/webhook, cancel/dispute). |
| `zod` | **Done** | Adopted in `validation.mjs` |
| Legacy `hosts` table | Low | Unused; drop it |
| **`server.mjs` modularization (REFACTOR)** | **✅ Done, July 1, 2026 (route split)** | `server.mjs` (3,092 lines at the time) split into 13 `routes/*.mjs` Fastify plugins + `authz.mjs` (shared auth/RBAC helpers, logger injected via `setAuthzLogger`) + `services.mjs` (Stripe/Resend singletons) + `sweeps.mjs` (all 4 keeper sweeps, started via `startSweeps(fastify)`); `server.mjs` is now ~97 lines of bootstrap. Route bodies extracted **verbatim** (byte-verified against the original); route inventory identical pre/post (63 routes, method+path diff empty); eslint `no-undef` clean (it caught one real cross-domain dep: `BPS_DENOM`, now exported from `routes/resale.mjs` to `routes/host.mjs`). Deployed and live-verified (Railway boot clean, `/health` + `/properties` serving). **Still open from the original item:** move the large inline HTML email strings into a `templates/` dir. |
...
| **M6b** — Stripe/Seal identity parity | **✅ Done, July 1, 2026.** Follow-up to M6: a card-paid booking never produces a guest-signed escrow (no guest wallet transaction happens anywhere in the Stripe flow), so host.jsx's "View Guest Identity" — gated on-chain by `seal_approve`, which requires a real `&BookingEscrow<T>` object — had nothing to check against for Stripe guests, even though the guest-side upload (Walrus + Seal encryption) already worked identically regardless of payment method. Considered and rejected an off-chain fallback (host.jsx's DB-based `hostManagesBooking()` check standing in for the on-chain gate): that would mean ARIA's backend, not Seal's key servers, deciding PII access — real custody, a genuine architecture regression, not a shortcut. Shipped instead: `createIdentityAttestationEscrow()` (`escrow.mjs`) — the existing zero-privilege auto-release key calls the **same, unmodified** `create_escrow` entry function used by the guest-signed path, funded with a trivial symbolic amount, `guest`/`host` set to the booking's real addresses. No Move contract changes, no redeployment. **Live-tested incident:** the first deploy set expiry_ms ~1 year out to keep the object away from `auto_release`; the contract hard-caps `expiry_ms` at `MAX_EXPIRY_MS` (`escrow.move` — exactly 30 days) and aborts with `EExpiryTooFar` past that, which the code correctly caught and returned `null` for (no bad state written — host.jsx's "View Guest Identity" surfaced a clean "No active escrow" error, confirming the fail-closed design worked as intended) rather than silently granting broken access. Fixed by setting expiry to 29 days (a day of margin under the cap). **Second live-tested incident, same test session:** after the expiry fix, `create_escrow` succeeded on-chain but host.jsx's "View Guest Identity" then failed with "Object ... not found" — traced (via Suiscan, then the transaction's own on-chain `EscrowCreated` event) to a real extraction bug, not lag: `autoReleaseKeypair.signAndExecuteTransaction()` (`@mysten/sui`'s `Keypair.signAndExecuteTransaction`) hardcodes its own `include: { transaction: true, effects: true }` internally and silently drops whatever `include` the caller passes, so the `objectTypes: true` this function requested never actually took effect — every call fell through to the "last Created entry in `changedObjects`" heuristic (`extractCreatedObjectId`), and on this test that heuristic picked a *different* incidental "Created" entry in the same PTB, not the real escrow. The wrong id got written to `identity_attestation_object_id`; Seal correctly failed to find it since it wasn't the real escrow object (that object was fine all along — the wrong id ARIA stored just happened to also get consumed/deleted shortly after by something unrelated, which is what showed up as "This object was deleted" on Suiscan and briefly pointed the investigation the wrong way). Fixed by bypassing the keypair convenience wrapper — build/sign/execute manually via `suiClient.core.executeTransaction()` so `events` can actually be requested — and reading `escrow_id` straight off the contract's own `EscrowCreated` event as the primary source of truth, with the `objectTypes` map and the old heuristic demoted to fallback-only. **Confirmed live, July 1, 2026:** fresh Stripe booking → webhook → attestation object created with the correct `escrow_id` → host's "View Guest Identity" successfully decrypted the guest's full PII client-side via Seal (name, DOB, phone, ID, address) — ARIA's backend never touched the plaintext, matching the whole design's premise. **Third incident, same session (deploy/tooling, not app logic):** while pushing the fix above, a `git commit` mistakenly run from the assistant's own sandbox shell landed for real in this repo — the sandbox path turned out to be a live mount of the same working directory, not an isolated copy as assumed — and captured a corrupted/truncated snapshot of 18 files (a filesystem-buffering quirk specific to that sandbox's view of large files) that got pushed to `origin/main` and deployed, crash-looping the Railway service (`SyntaxError: Unexpected end of input` in `server.mjs`) for a few minutes. Root-caused by comparing the live file (correct) against the deployed one (truncated) and the git log (confirmed the sandbox commit's hash was genuinely `HEAD`). Fixed by committing the real working-tree contents (already correctly staged by `git add -A`) from PowerShell instead, after clearing a stale `.git/HEAD.lock` the sandbox process left behind. No application data or on-chain state was affected — this was a deploy-pipeline incident only. Lesson: git operations on this repo must only run from the user's own terminal going forward, never from the assistant's sandbox shell. **Known limitation, not yet closed (separate from the above):** unlike a SuiUSD booking's escrow, this means Stripe-booking identity access isn't indefinite — past ~29 days the object becomes a permissionless `auto_release` target for anyone, after which `seal_approve` has nothing left to check against. Before mainnet, add either a renewal sweep or a lazy-create fallback in `/host/guest-identity` itself. Called from `confirmCardBooking()` (`bookings.mjs`) right after a Stripe booking is verified-confirmed; best-effort — a failure here can't un-confirm an already-charged booking, it just means that host won't get identity access for that one booking, surfaced as a clear error rather than any silent fallback. The resulting object id is stored in a new, dedicated `identity_attestation_object_id` column (`db.mjs`) — never `escrow_object_id` — so `autoReleaseEscrow`, the deposit sweep, refunds, resale, and the check-in-pass flow can't see or touch it; `GET /host/guest-identity/:bookingRef` (`server.mjs`) now accepts either column as the on-chain object to check identity access against, SuiUSD bookings unchanged. Also fixed a related bug found while wiring this: `createPendingCardBooking()`'s INSERT omitted `deposit_status`, so it silently took the table's `'held'` default instead of `NULL` — now set explicitly. Testnet-only design note: the auto-release key needs a small amount of testnet SUI beyond gas to fund these; not a concern per-user direction, since no real funds (including stablecoins) exist on testnet — revisit funding/rate-limiting this key specifically before any mainnet migration. |
...
| **R1 (full split)** | **✅ Done, July 1, 2026** — `server.mjs` → 13 `routes/*.mjs` plugins + `authz.mjs`/`services.mjs`/`sweeps.mjs`; see the modularization row in the tech-debt table above. Email `templates/` extraction still open. |
...
- **`server.mjs` monolith (~1,500 lines)** = **R1** — ✅ split into `routes/*.mjs`
  July 1, 2026; the inline-HTML-email → templates move (the reviewer's specific
  add) is still open.
