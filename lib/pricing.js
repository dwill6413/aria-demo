...
// refundable deposit). The Stripe Checkout path (server.mjs /payment/create-
// intent) doesn't charge a deposit at all — see bookings.mjs's
// createPendingCardBooking for why — so this stayed matched to what the
// backend actually charges instead of a number the guest would never really
// pay. server.mjs computes this exact formula server-side before creating
// the Checkout Session, so what's shown here is what gets charged.
export const getCardTotal    = (p, n) => (getBookingTotal(p, n) * 1.029 + 0.30).toFixed(2);
export const getCardFee      = (p, n) => (getBookingTotal(p, n) * 0.029 + 0.30).toFixed(2);
