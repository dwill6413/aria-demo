...
async function fetchCurrentEpoch() {
  ...
  const epoch = parseInt(data?.result?.epoch, 10);
...
export async function submitSignedTransaction(transactionBytesBase64, signature) {
  const res = await fetch(FULLNODE_URL, {
    ...
    body: JSON.stringify({
      ...
      params: [transactionBytesBase64, [signature], { showEffects: true }, 'WaitForLocalExecution'],
    }),
  });
  const data = await res.json();
  if (data.error) {
    throw new Error(data.error.message || 'Transaction submission to Sui failed');
  }
  const digest = data?.result?.digest;
