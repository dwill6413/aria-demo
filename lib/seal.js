...
// The package id used for BOTH encryption and the seal_approve decrypt call.
// IMPORTANT: Seal embeds this id in the encrypted blob and, at decrypt time,
// requires the seal_approve PTB to target the SAME id ("Package ID used in PTB
// is invalid" otherwise). seal_approve only EXISTS in the v4 upgrade, so the
// PTB must target v4 — which means encryption must use v4 too. We do NOT use the
// original/type-defining id here: Seal normalizes to the first-published id
// internally on the key-server side (fetch_first_pkg_id) for key derivation, so
// using the current (v4) id consistently is both correct and required.
export const CURRENT_PACKAGE_ID =
  process.env.NEXT_PUBLIC_ESCROW_PACKAGE_ID || ORIGINAL_PACKAGE_ID;
...
const WALRUS_PUBLISHER = 'https://publisher.walrus-testnet.walrus.space/v1/blobs?epochs=3';
...
export async function encryptAndStorePII(suiClient, guestAddress, piiObject) {
  ...
  const { encryptedObject } = await sealClient(suiClient).encrypt({
    ...
    packageId: CURRENT_PACKAGE_ID, // must match the seal_approve PTB target (v4)
