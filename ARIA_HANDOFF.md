...
1. ~~**zkLogin salt shared globally**~~ — **RESOLVED and verified live, July 1, 2026 (M3).** Used to be one `ZKLOGIN_SALT`/`NEXT_PUBLIC_ZKLOGIN_SALT` value shared by every user; changing it re-derived every address at once, which actually happened June 30, 2026 and caused real confusion July 1 when two test accounts' addresses had silently swapped. Fixed with a `user_salts` table (one row per Google `sub`, created on first login, never changed after) — see `getOrCreateUserSalt`/`handleZkLoginSalt` in `auth.mjs`, `POST /auth/zklogin/salt`, and `ARIA_KEY_INVENTORY.md` §8 for the full implementation. The legacy shared value now only seeds a brand-new user's first row; changing it can no longer reshuffle anyone who's already logged in. Confirmed post-deploy: both `cwilliams36092@gmail.com` and `ariasuidemo@gmail.com` logged in and each kept the exact same address they had before the rollout. Still true regardless: don't change the legacy value at mainnet migration anyway (no reason to) — same salt + same Google OAuth client ID carries every address from testnet to mainnet unchanged; a new OAuth client ID would still re-derive everything since `aud` is also a derivation input.
